...
interface Episode {
  name: string; // Thêm name để giữ nguyên giá trị gốc như "1A", "1B"
  number: number;
  title: string;
  duration?: string;
  isWatched?: boolean;
...
const MovieDetail = ({ movie }: MovieDetailProps) => {
  ...
  // Parse và giữ nguyên name để tránh lọc trùng lặp không mong muốn
    ...
    : movie.meta?.ophim_episode_list?.[0]
      ? (() => {
          try {
            ...
            const seenEpisodes = new Set<string>(); // Theo dõi dựa trên name và link_m3u8

            Object.values(phpData).forEach((server: any) => {
              const serverEpisodes: Episode[] = [];

              Object.values(server.server_data || {}).forEach((ep: any) => {
                const episodeKey = `${server.server_name}-${ep.name}-${ep.link_m3u8}`; // Sử dụng name gốc
                if (!seenEpisodes.has(episodeKey)) {
                  seenEpisodes.add(episodeKey);
                  serverEpisodes.push({
                    name: ep.name, // Giữ nguyên name như "1A", "1B"
                    number: parseInt(ep.name.replace(/[A-Za-z]/g, '') || '1'), // Lấy số từ name (loại bỏ chữ cái)
                    title: ep.filename,
            ...
            <div className="lg:col-span-3">
              ...
              >
                ...
                {movie.meta?.ophim_total_episode?.[0] === "1" ? (
                  ...
                  episodes_list.map((server, serverIdx) => (
                    <div key={`server-${serverIdx}`} className="mb-6">
                      <Badge
                        variant="outline"
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-8 gap-2 mt-4">
                        {server.episodes.map((ep, epIdx) => (
                          <button
                            key={`${serverIdx}-${epIdx}`} 
                            onClick={() => {
                              setSelectedEpisode(ep.number);
                              router.push(`/movie/${slug}/tap-${ep.name}-server-${serverIdx}`); 
                            }}
